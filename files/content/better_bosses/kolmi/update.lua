...
function phase_melee()
	...
	GamePlaySound( "data/audio/Desktop/animals.bank", "animals/generic/attack_bite", pos_x, pos_y )

	local targets = EntityGetInRadiusWithTag( pos_x, pos_y, 30, "mortal" )
	for k=1, #targets
	do local v = targets[k]
		if v ~= this then
			local damagemod = EntityGetFirstComponentIncludingDisabled(v, "DamageModelComponent")
			if damagemod ~= nil then
				local health = ComponentGetValue2(damagemod, "hp")
				local damage = health / 4
				EntityInflictDamage( v, damage, "DAMAGE_SLICE", GameTextGetTranslatedOrNot("animal_boss_centipede") .. "'s big appetite", "NONE", 0, 0, entity_id )
			end
		end
	end
...
function next_phase()
	...
	if #players > 0 then
		...
		if GameHasFlagRun("fair_gameshow_kolmi") then
			set_logic_state( states.MoveDirectlyTowardsPlayer )
			...
			local physicsshape = EntityGetFirstComponent( entity_id, "PhysicsShapeComponent" )
			if (physicsshape ~= nil) then
				ComponentSetValue2( physicsshape, "radius_x", 0 )
				ComponentSetValue2( physicsshape, "radius_y", 0 )
			end
		end
...
function set_logic_state( state )
	...
	if (ai ~= nil) then
		old = tonumber(ComponentGetValue2( ai, "state" ))
	end
...
function bite_eye()
	...
	set_main_animation("bite", get_idle_animation_name())
	GameEntityPlaySound( GetUpdatedEntityID(), "open_mouth" )
	is_eye_open = false
	boss_wait(27)
end
