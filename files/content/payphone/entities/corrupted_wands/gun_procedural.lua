...
local actions_by_level = {}

for i, action in ipairs(actions) do
	if action.tm_trainer then
		local spawn_levels = action.spawn_level
		local action_type = action.type
		-- split spawn levels by comma
		local levels = {}
		for level in string.gmatch(spawn_levels, "([^,]+)") do
			table.insert(levels, tonumber(level))
		end

		for i, level in ipairs(levels) do
			if actions_by_level[level] == nil then actions_by_level[level] = {} end
			table.insert(actions_by_level[level], action)
		end

		if action_by_type[action_type] == nil then action_by_type[action_type] = {} end

		for i, level in ipairs(levels) do
			if action_by_type[action_type][level] == nil then action_by_type[action_type][level] = {} end
			table.insert(action_by_type[action_type][level], action)
		end
	end
end

GetRandomAction = function(x, y, max_level, i)
	local level = Random(1, max_level)
	local action = nil
	local actions = actions_by_level[level]

	if actions ~= nil then action = actions[Random(1, #actions)].id end

	return action
end

GetRandomActionWithType = function(x, y, max_level, type, i)
	local level = Random(1, max_level)
	local action = nil
	local actions = action_by_type[type][level]
...
function init_total_prob(value)
	...
	for i, v in ipairs(value) do
		if v.prob ~= nil then value.total_prob = value.total_prob + v.prob end
	end
end
...
function get_gun_probs(what)
	...
	if gun_probs[what] == nil then return nil end

	if gun_probs[what].total_prob == 0 then init_total_prob(gun_probs[what]) end
...
function apply_random_variable(t_gun, variable)
	...
	if variable == "deck_capacity" then
		...
		if t_gun["force_unshuffle"] == 1 then
			min = 1
			max = ((cost - 15) / 5)
			if max > 6 then max = 6 + ((cost - (15 + 6 * 5)) / 10) end
		end
...
function GetWand(gun)
	...
	for k, wand in pairs(wands) do
		...
		if score <= best_score then
			...
			if score == 0 and Random(0, 100) < 33 then return best_wand end
		end
	end
	return best_wand
end
...
function get_gun_data(cost, level, force_unshuffle)
	...
	if level == 1 then
		if Random(0, 100) < 50 then cost = cost + 5 end
	end
	...
	if gun["mana_max"] < 50 then gun["mana_max"] = 50 end
	...
	-- this is so boring
	if Random(0, 10000) <= 9999 then gun["deck_capacity"] = clamp(gun["deck_capacity"], 2, 26) end

	if gun["deck_capacity"] <= 1 then gun["deck_capacity"] = 2 end

	if gun["reload_time"] >= 60 then
		function random_add_actions_per_round()
			gun["actions_per_round"] = gun["actions_per_round"] + 1
			if Random(0, 100) < 70 then random_add_actions_per_round() end
		end
...
function wand_add_random_cards(gun, entity_id, level)
	...
	if is_rare == 1 then good_cards = good_cards * 2 end
	...
	if Random(0, 100) < 10 or is_rare == 1 then card_count = card_count + Random(1, 2) end
	...
	if Random(0, 100) < (orig_level * 10) - 5 then random_bullets = 1 end
