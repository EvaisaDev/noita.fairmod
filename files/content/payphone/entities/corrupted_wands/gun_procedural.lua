...
for i, action in ipairs(actions)do
	if(action.tm_trainer)then
		...
		for i, level in ipairs(levels)do
			if(actions_by_level[level] == nil)then
				actions_by_level[level] = {}
			end
			table.insert(actions_by_level[level], action)
		end

		if(action_by_type[action_type] == nil)then
			action_by_type[action_type] = {}
		end

		for i, level in ipairs(levels)do
			if(action_by_type[action_type][level] == nil)then
				action_by_type[action_type][level] = {}
			end
...
GetRandomAction = function(x, y, max_level, i)
	...
	if(actions ~= nil)then
		action = actions[Random(1, #actions)].id
	end
