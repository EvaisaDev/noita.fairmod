...
do
    ...
    for k=0,new_enemy_count do
        ...
        for ui_roll=1,ui_rolls do
            ...
            for x=x_pos,x_size do
                for y=y_pos,y_size do
                    ...
                    if colour ~= 0 then
                        ModImageSetPixel( output_icon, x, y, colour ) 
                    end
                end
            end
        end
        ...
        do
            ...
            local opts = {"scavenger_clusterbomb","scavenger_glue","scavenger_grenade","scavenger_heal","scavenger_invis","scavenger_leader","scavenger_mine","scavenger_poison","scavenger_shield","scavenger_smg","ant","roboguard","roboguard_big","phantom_a","phantom_b","alchemist","ghoul","spitmonster","wizard_wither","zombie","turret","barfer","skullfly","maggot_tiny_head"}
            ...
            for sprite_roll=1,sprite_rolls do
                ...
                local output_sprite = ModImageIdFromFilename( table.concat({"mods/noita.fairmod/files/content/corrupted_enemies/gfx/noita.fairmod_enemy_corrupted_0",k,".png"}))

                for grid_x=1,6 do
                    for grid_y=1,11 do
                        for x=x_pos,x_size do
                            for y=y_pos,y_size do
                                local colour = ModImageGetPixel( victim_sprite, x+((grid_x - 1) * 20), y+((grid_y - 1) * 21) )
                                if colour ~= 0 then
                                    ModImageSetPixel( output_sprite, x+((grid_x - 1) * 20), y+((grid_y - 1) * 21), colour ) 
                                end
                            end
                        end
                    end
        ...
        local nouns = {
            ...
            "lion", "storm", "shadow", "blade", "falcon", "giant", "tiger", "whisper", 
            "raven", "phoenix", "hawk", "serpent", "guardian", "flame", "warrior", "spirit"
        }
        local suffixes = {
            "destruction", "slaughter", "fury", "rage", "vengeance", "chaos", "wrath", "conquest", 
            "doom", "reckoning", "havoc", "ruin", "madness", "torment", "devastation", "annihilation", 
            "ascension", "rebirth", "domination", "obliteration", "eternity", "sorrow", "whispers", "obliteration",
            "silence", "awakening", "judgment", "shadows", "secrets", "dawn"
        }

        local suffix = ""
        if math.random(1,3) == 1 then
            suffix = table.concat({" of ",suffixes[math.random(1,#suffixes)]})
        end
        local name = table.concat({adjectives[math.random(1,#adjectives)]," ",nouns[math.random(1,#nouns)],suffix})
        ...
        --Generate spawn file
        do
            local content = ModTextFileGetContent("mods/noita.fairmod/files/content/corrupted_enemies/spawnpool_append.lua")
            content = content:gsub("k", k)
            ModTextFileSetContent(table.concat({"mods/noita.fairmod/files/content/corrupted_enemies/spawnpool_append_0",k,".lua"}), content)
        end
