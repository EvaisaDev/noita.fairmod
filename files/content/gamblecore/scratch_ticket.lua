...
local scratch_ticket = {
    new = function(x, y)
        ...
        self.draw = function(self)
            ...
            local background_width, background_height = GuiGetImageDimensions(self.gui, "mods/noita.fairmod/files/content/gamblecore/scratchoff.png", 1.4)

            local x, y = screen_width / 2 - background_width / 2, screen_height / 2 - background_height / 2

            GuiImage(self.gui, new_id(), x, y, "mods/noita.fairmod/files/content/gamblecore/scratchoff.png", 1, 1.4, 1.4)
            ...
            if mx and my then
                if mouse_pressed then
                    if mx >= scratch_area_x and mx <= scratch_area_x + scratch_area_width and my >= scratch_area_y and my <= scratch_area_y + scratch_area_height then
                            ...
                            for _, point in ipairs(line_points) do
                                ...
                                for x_pix = scratch_x - 4, scratch_x + 4 do
                                    for y_pix = scratch_y - 4, scratch_y + 4 do
                                        if x_pix >= 0 and x_pix < scratch_area_width and y_pix >= 0 and y_pix < scratch_area_height then
            ...
            for i, scratch_number in ipairs(self.scratch_numbers) do
				...
				if scratch_count >= (cell_width * cell_height) / 1.4 then
					cell_cleared = true
				end
				local is_winning_number = false
				if self.winner_lookup[scratch_number.number] then
            ...
            for y_pix = 0, scratch_area_height - 1 do
                ...
                while x_pix < scratch_area_width do
                    if not self.scratched_pixels[x_pix] or not self.scratched_pixels[x_pix][y_pix] then
                        ...
                        local run_length = 1
                        x_pix = x_pix + 1
                        while x_pix < scratch_area_width and (not self.scratched_pixels[x_pix] or not self.scratched_pixels[x_pix][y_pix]) do
                        ...
                        GuiImage(self.gui, new_id(), draw_x, draw_y, "mods/noita.fairmod/files/content/gamblecore/scratch_pixel.png", 1, width, height)
                    else
                        x_pix = x_pix + 1
                    end
                end
