...
if next(action_info_map) == nil then
    ...
    for _, action in ipairs(actions) do
        ...
        if not action_info_map[action_type] then
            action_info_map[action_type] = {}
        end
...
local function create_tmtrainer_action(action_type, index)
    ...
    if primary_action then
        table.insert(added_actions, primary_action)
    end
    ...
    local new_action = {
        ...
        action = function(recursion_level, iteration)
            ...
            if not success then
                print("TMTRAINER ERROR: " .. tostring(err))
            end
        end,
    }
