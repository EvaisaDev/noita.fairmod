...
if (#(EntityGetComponent(item, "PhysicsBodyComponent") or {}) > 0) then
    is_physics = true
end
...
if (not is_physics) then
    ...
    if (player ~= nil) then
        ...
        if (isNearPlaform()) then
            PhysicsApplyForce(item, rotated_direction_x * speed, rotated_direction_y * speed)
        end
