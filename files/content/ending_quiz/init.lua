...
do
	...
	content = content:gsub(
		"data/entities/animals/boss_centipede/ending/ending_sampo_spot_underground%.xml",
		"mods/noita.fairmod/files/content/ending_quiz/fake_ending.xml"
	)
...
do
	...
	local content = ModTextFileGetContent(path)
	content =
		content:gsub('GamePlaySound%( "data/audio/Desktop/event_cues%.bank", "event_cues/midas/create", x, y %)', "")
	ModTextFileSetContent(path, content)
end
...
module.spawn_shape = function(x, y)
	...
	if shape_chosen == "null" then
		...
		EntityLoad(
			table.concat({ "mods/noita.fairmod/files/content/ending_quiz/shape/", shape, ".xml" }),
			x + 140,
			y - 40
		)
