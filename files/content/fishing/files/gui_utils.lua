...
function hex2argb(hex)
	...
	return bit.band(bit.rshift(n, 24), 0xFF) / 255,
...
function AutoBox(gui, autobox_id, x, y, elem1, elem2, elem3)
	...
	if type(elem1) == "function" then callback = elem1 end
	if type(elem2) == "function" then callback = elem2 end
	if type(elem3) == "function" then callback = elem3 end
		...
		Horizontal(gui, x, y - y_reverse, ui_scale, function()
			for col = 1, row_length do
				...
				Horizontal(gui, offset_x * (col - 1), 0, ui_scale, function()
					...
					if vars.padding_x then HorizontalSpacing(gui, vars.padding_x) end
				end)
			end
		end)

		if vars.padding_y then VerticalSpacing(gui, vars.padding_y) end
	end
end
...
function DrawText(gui, font, x, y, scale, z_index, in_string, color, margin_x, margin_y)
	...
	for line in in_string:gmatch("[^\r\n]+") do
		...
		for _, c in ipairs(UTF8ToCharArray(line)) do
			...
			if conversion[c] ~= nil then letter = conversion[c] end
