...
dofile("mods/noita.fairmod/files/content/fishing/files/eva_utils.lua")
...
local dialog_system = {
  ...
  dialog_sounds = {
	default = {
		{ bank = "data/audio/Desktop/ui.bank", event = "ui/button_select" },
	}
  },
  char_sounds = {
	default = {
		{char = " ", bank = "data/audio/Desktop/ui.bank", event = "ui/button_select" },
	}
  },
...
local last_sound = last_sound or nil
...
dialog_system.open_dialog = function(message)
  ...
  dialog.show = function(message)
	...
	local previous_message_dialog_sound = dialog.message.dialog_sound
	local previous_message_char_sound = dialog.message.char_sound
	local previous_message_talk_string = dialog.message.talk_string
	local previous_message_every_x_char = dialog.message.every_x_char
	...
	dialog.message.dialog_sound = message.dialog_sound or previous_message_dialog_sound
	dialog.message.char_sound = message.char_sound or previous_message_char_sound
	dialog.message.talk_string = message.talk_string or previous_message_talk_string
	dialog.message.every_x_char = message.every_x_char or previous_message_every_x_char
    dialog.lines = {{}}
    dialog.current_line = dialog.lines[1]
  ...
  routines.gui = async(function()
    ...
    while is_open do
	  ...
	  if(EntityHasFlag(entity_id, "player_bought"))then
		if(dialog.message.shop_override ~= nil)then
			dialog.message.shop_override(dialog)
			EntityRemoveFlag(entity_id, "player_bought")
		end
	  end
      ...
      if dialog.fade_in_portrait > -1 then
        ...
        if dialog.message.name and dialog.message.name ~= "" then
          ...
          GuiZSetForNextWidget(gui, 2)
          GuiImageNinePiece(gui, 5, screen_width/2 - width/2, (screen_height - dialog_system.dialog_box_y - height - nameplate_height - 3) - (dialog.message.nametag_offset or 0), nameplate_inner_width, nameplate_height)
          local diff = nameplate_inner_width - name_width + nameplate_padding
          GuiText(gui, screen_width/2 - width/2 + diff/2, (screen_height - dialog_system.dialog_box_y - height - nameplate_height/2 - name_height + nameplate_padding)  - (dialog.message.nametag_offset or 0), dialog.message.name)
        end
      end
      -- Render text
      local y_offset = 0
  ...
  routines.logic = async(function()
	...
	local dialog_sound = dialog.message.dialog_sound
	local char_sound = dialog.message.char_sound
	local talk_string = dialog.message.talk_string
	local every_x_char = dialog.message.every_x_char or 1
    local i = 1

	local x_chars = {}

	if dialog_sound ~= "none" and dialog_sound ~= nil then

		local instance = dialog_system.dialog_sounds[dialog_sound or "default"][Random(1, #(dialog_system.dialog_sounds[dialog_sound or "default"]))]
    ...
    while i <= #dialog.message.text do
	  ...
	  --[[
	  if(talk_string ~= nil and i % every_x_char == 0)then
		if(i - string.len(talk_string) >= 0)then
			if(dialog.message.text:sub(i - string.len(talk_string), i) ~= talk_string)then
				if dialog_sound ~= "none" and dialog_sound ~= nil then

					local allowed_sounds = {}

					for k, v in pairs(dialog_system.dialog_sounds[dialog_sound or "default"])do
						if(last_sound == nil or v.event ~= last_sound.event or v.bank ~= last_sound.bank)then
							table.insert(allowed_sounds, v)
						end
					end


					local instance = allowed_sounds[Random(1, #allowed_sounds)]
	  ...
	  if(x_chars[char] == nil)then
		x_chars[char] = every_x_char
	  else
		x_chars[char] = x_chars[char] + 1
	  end

	  if char_sound ~= "none" and frame_last_played_sound2 ~= GameGetFrameNum() and x_chars[char] % every_x_char == 0 then
        frame_last_played_sound2 = GameGetFrameNum()

		local allowed_sounds = {}

		local was_found = false

		for k, v in pairs(dialog_system.char_sounds[char_sound or "default"])do
			if(v.char == char)then
				was_found = true
				if(last_sound == nil or v.event ~= last_sound.event or v.bank ~= last_sound.bank)then
		...
		if(#allowed_sounds == 0 and was_found)then
			table.insert(allowed_sounds, last_sound)
		end

		if(#allowed_sounds ~= 0)then
			local instance = allowed_sounds[Random(1, #allowed_sounds)]
      ...
      if typing_sound ~= "none" and play_sound and frame_last_played_sound ~= GameGetFrameNum() then
		...
		local typing_sound_table = {bank = "", event = ""}

		if(dialog_system.sounds[typing_sound or "default"].bank == nil)then
			local instance = dialog_system.sounds[typing_sound or "default"][Random(1, #(dialog_system.sounds[typing_sound or "default"]))]

			typing_sound_table.bank = instance.bank
			typing_sound_table.event = instance.event
		else
			typing_sound_table.bank = dialog_system.sounds[typing_sound or "default"].bank
			typing_sound_table.event = dialog_system.sounds[typing_sound or "default"].event
		end

        local bank = typing_sound_table.bank
        local event = typing_sound_table.event
        GamePlaySound(bank, event, 0, 0)
      end
