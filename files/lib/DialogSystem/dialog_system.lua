...
dialog_system.open_dialog = function(message)
	...
	dialog.close = function(on_closed_callback)
		...
		if routines.logic then routines.logic.stop() end
		dialog.closing = true
		dialog.lines = { {} }
		dialog.current_line = dialog.lines[1]
		dialog.show_options = false
		if dialog.on_closing and type(dialog.on_closing) == "function" then dialog.on_closing() end
	...
	routines.gui = async(function()
		if dialog_system.disable_controls then set_controls_enabled(false) end
		while is_open do
			...
			if dialog.is_too_far() then dialog.close() end
			GuiStartFrame(gui)
			...
			GuiImageNinePiece(
				...
				screen_height - dialog_system.dialog_box_y - dialog_system.dialog_box_height / 2 - height / 2,
				width,
				height
			)
			if dialog.fade_in_portrait > -1 then
				...
				if dialog.message.portrait_overlay then
					...
					GuiImage(
						...
						x,
						y,
						dialog.message.portrait_overlay,
						1,
						1,
						1,
						0,
						GUI_RECT_ANIMATION_PLAYBACK.Loop,
						message.animation_overlay or ""
					)
					GuiIdPop(gui)
				end
				...
				GuiImage(
					...
					1,
					0,
					GUI_RECT_ANIMATION_PLAYBACK.PlayToEndAndPause,
					"anim_" .. tostring(dialog.fade_in_portrait)
				)
				GuiZSetForNextWidget(gui, -1)
				...
				if dialog.message.name and dialog.message.name ~= "" then
					...
					GuiImageNinePiece(
						...
						5,
						screen_width / 2 - width / 2,
						screen_height - dialog_system.dialog_box_y - height - nameplate_height - 3,
						nameplate_inner_width,
						nameplate_height
					)
			...
			for i, line in ipairs(dialog.lines) do
				...
				for i2, char_data in ipairs(line) do
					...
					if char_data.shake then
						...
						if char_data.img then
							...
							GuiImage(
								...
								i2,
								-3,
								(i - 1) * line_height + wave_offset_y,
								get_image(char_data.img),
								0,
								1,
								1,
								0,
					...
					if char_data.wave then wave_offset_y = math.sin(char_i * 0.5 + GameGetFrameNum() * 0.1) * 1 end
					if char_data.rainbow then
						local color = Color:new((char_i * 25 + GameGetFrameNum() * 5) % 360, 0.7, 0.6)
						r, g, b = color:get_rgb()
					end
					if char_data.blink then a = math.sin(GameGetFrameNum() * 0.2) * 0.3 + 0.7 end
					if char_data.img then
						...
						GuiImage(
							gui,
							i2,
							(absolute_position and 0 or -3) + shake_offset.x,
							(absolute_position and 0 or y_offset) + wave_offset_y + shake_offset.y,
							get_image(char_data.img),
							a,
							1,
							1,
						...
						GuiIdPop(gui)
						if not absolute_position then GuiText(gui, -2, 0, "") end
					else
						GuiColorSetForNextWidget(gui, r, g, b, a)
						-- There is a bug with chinese/english where a single space doesn't have any width, i.e. GuiGetTextDimensions(gui, " ") == 0
						if GuiGetTextDimensions(gui, char_data.char) == 0 then
							GuiText(
								gui,
								(absolute_position and 0 or -2) + shake_offset.x,
								(absolute_position and 0 or y_offset) + wave_offset_y + shake_offset.y,
								"  "
							)
						else
	...
	routines.logic = async(function()
		...
		while i <= #dialog.message.text do
			...
			elseif char == "{" and prev_char ~= "\\" then
				...
				if command then
					if command == "delay" then
						...
						if delay < 0 then skip_char_count = math.abs(delay) end
			...
			if skip_dialogue then do_wait = false end
		...
		if not skip_dialogue then wait(15) end
